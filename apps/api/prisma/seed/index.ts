...
async function main() {
  ...
  // analytics (profit = sellingPrice − costPrice). Kalpesh confirmed these
  // estimates are reasonable to start with; can be corrected later against
  // real supplier invoices, same as basePrice/mrp were corrected earlier.
  ...
  const tiers: Record<string, RuleTier> = {
    "cat-grocery": { floorPct: 0.95, ceilPct: 1.3, surgeMax: 1.3 }, // essentials — tight ceiling
    "cat-dairy": { floorPct: 0.95, ceilPct: 1.25, surgeMax: 1.25 }, // perishables — tightest
    "cat-snacks": { floorPct: 0.9, ceilPct: 1.6, surgeMax: 1.6 },
    "cat-beverages": { floorPct: 0.9, ceilPct: 1.75, surgeMax: 1.75 },
    "cat-instant": { floorPct: 0.92, ceilPct: 1.5, surgeMax: 1.5 },
  };

  let ruleCount = 0;
  for (const store of stores) {
    for (const product of products) {
      const tier = tiers[product.categoryId] ?? {
        floorPct: 0.9,
        ceilPct: 1.5,
        surgeMax: 1.5,
      };
      await prisma.pricingRule.upsert({
        ...
        create: {
          ...
          floorPrice: parseFloat(
            (product.basePrice * tier.floorPct).toFixed(2),
          ),
