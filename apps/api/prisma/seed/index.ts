...
async function main() {
  ...
  type StockEntry = { qty: number; reorderLevel: number; reorderQty: number; currentPrice: number };

  const stockMatrix: Record<string, Record<string, StockEntry>> = {
    'store-mumbai-bandra': {
  ...
  for (const store of stores) {
    for (const product of products) {
      const s = stockMatrix[store.id]?.[product.id];
      if (!s) continue;
