...
// Blocks rapid repeated clicks on simulator buttons — protects the
// database and Render's free-tier resources from being hammered by
// accidental button-mashing during a demo.
// Covers +100/+500/+1000/DDoS buttons, which all call /inject. DDoS fires
// 5 rapid calls on its own, so the limit allows a full DDoS run plus a
// few manual clicks within the same minute.
const injectLimiter = rateLimit({
  windowMs: 5 * 60 * 1000, // 5 minutes
  max: 8,
  message: {
    error: "Too many simulator requests. Please wait a moment and try again.",
  },
  standardHeaders: true,
  legacyHeaders: false,
});

// Reset Demo is a separate, heavier action — kept at a lower limit.
 ...
 *  so it can be reused by both /inject (single call) and
 ...
 *
 * now also passes through cappedAtMrp from the pricing engine,
...
async function injectForProduct(
  ...
  //  record this as a real sale, using the price BEFORE the
 ...
 * — scripted, repeatable spike demo.
 *
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // ── Step 0: Reset ──
      const deleted = await prisma.demandEvent.deleteMany({
        where: { storeId },
      });
      const redis = getRedis();
      ...
      async function computeActualSurgePercent(): Promise<{
        ...
        const results = await Promise.all(
          allProductIds.map((pid) =>
            computeSuggestedPrice({ storeId, productId: pid }).catch(
              () => null,
            ),
          ),
        );
 ...
 *
 *  follow-up: also broadcasts price-update + stock-update over
 ...
 *
 *  (MRP pass): broadcast now also includes mrp + cappedAtMrp so
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!baseline) {
        return res
          .status(400)
          .json({ error: "No baseline data for this store" });
      }
