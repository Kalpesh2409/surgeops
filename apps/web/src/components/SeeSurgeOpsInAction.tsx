...
function StepCard({
  ...
  return (
    <motion.div
      initial={{ opacity: 0.3, scale: 0.96, y: 20 }}
      whileInView={{ opacity: 1, scale: 1, y: 0 }}
      viewport={{ once: false, amount: 0.5 }}
      transition={{ duration: 0.6, ease: "easeOut" }}
      className="bg-white/5 border border-white/10 rounded-xl p-6 max-w-md w-full mx-auto"
    >
      {children}
    </motion.div>
  );
  ...
  return (
    <div className="flex justify-center py-4">
      <motion.div
        ...
        className="w-px bg-gradient-to-b from-sky-500/50 to-transparent"
      />
    </div>
  );
}
...
function LiveDemandCounter() {
  ...
  return (
    <span className="text-2xl font-bold text-sky-400">{count}</span>
  );
}
    ...
    <section className="w-full bg-black text-white px-6 py-24">
      <div className="max-w-2xl mx-auto">
        ...
        <StepCard>
          ...
          <p className="text-sky-400 font-bold text-lg">ML baseline price: ₹42.00</p>
        </StepCard>
        ...
        <motion.p
          ...
          viewport={{ once: false, amount: 0.5 }}
          transition={{ duration: 0.7, ease: "easeOut" }}
          className="text-center text-gray-400 max-w-lg mx-auto mt-16"
        >
